...
The standalone `.exe` will appear in the `dist/` folder. The spec file embeds `app_icon.ico` as both the EXE icon and a bundled resource.
...
The icon source is `app_icon.svg` (generated by `generate_icon.py`). To recreate or modify the icon:

1. Edit `generate_icon.py` and run it to produce `app_icon.svg`
2. Open the SVG in a browser and take a 256×256 screenshot as `app_icon.png`
3. Run `crop_icon.py` to crop it to a circle and generate `app_icon.ico`

Requires [Pillow](https://pypi.org/project/Pillow/) (`pip install Pillow`) for the crop/ICO step.
